...
datatype 'a myTree = 
  Leaf 'a |
  Node 'a "'a myTree" "'a myTree"

fun getBottom :: "'a myTree \<Rightarrow> 'a" where
  "getBottom (Leaf x) = x" |
  "getBottom (Node x l r) = x"

context po
begin

fun rc_order :: "'a myTree \<Rightarrow> bool" where
  "rc_order (Leaf x) = True" |
  "rc_order (Node x l r) = (x \<sqsubseteq> (getBottom l) \<and> x \<sqsubseteq> (getBottom r) \<and> rc_order l \<and> rc_order r)"

typedef 'a::po rc = "{t::'a myTree. rc_order t}"
by (metis mem_Collect_eq rc_order.simps(1))
